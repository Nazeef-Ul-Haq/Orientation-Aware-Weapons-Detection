...
def bilinear_sampler_Interpol(input_fmap, x, y, iimap):
...
def transformer_layer_fMapSep(input_fmap, orient_scores, rpn_boxes, height, width, out_dims=None, **kwargs):
    ...
    outMap = np.zeros((len(rpn_boxes), 72,72, input_fmap.shape[3]), dtype = object)
    ...
    for idx in range(0, len(rpn_boxes)):
	    ...
	    #print ("input_fmap.shape",input_fmap.shape)
	        outMap[idx, 0:H, 0:W, 0:input_fmap.shape[3]] = input_fmap
	    ...
	    #print rpn_boxes[idx,1:5], [idx]+[rpn_boxes[idx,1],rpn_boxes[idx,2], rpn_boxes[idx,3], rpn_boxes[idx,4]]
	        tr_rotated_box_all.append([rpn_boxes[idx,0],rpn_boxes[idx,1], rpn_boxes[idx,2], rpn_boxes[idx,3]])
		...
		#ross = [[0]+ il for il in rotated_rpns]
                    ...
                    #print(imap)
                    #print(imap.shape)
                    #out_fmap = bilinear_sampler_Interpol(imap, x_s, y_s, input_fmap)
